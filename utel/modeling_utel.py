...
class PositionalEmbedding2D(nn.Module):
    ...
    def forward(self, bbox):
        # bbox: [seq_length, seq_length, batch_size, dim_bbox]
        stack = []
...
class LayoutLMEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.bbox_sinusoid_emb = PositionalEmbedding2D(
            dim_bbox_sinusoid_emb, dim_bbox=4
        )
        self.bbox_projection = nn.Linear(
            dim_bbox_sinusoid_emb, dim_bbox_projection, bias=False
        )

        self.seq_sinusoid_emb = PositionalEmbedding2D(
            dim_bbox_sinusoid_emb, dim_bbox=1
        )
    ...
    def calc_1d_pos_emb(self, position_ids):
        if position_ids is None:
            position_ids = self.position_ids[:, :512]
        # bbox_t: [seq_length, batch_size, dim_bbox]
        position_ids = position_ids.unsqueeze(dim=2)
        position_ids_t = position_ids.transpose(0, 1)

        # bbox_pos: [seq_length, seq_length, batch_size, dim_bbox]
        seq_pos = position_ids_t[None, :, :, :] - position_ids_t[:, None, :, :]
        # m=50 in default
        seq_pos = seq_pos.clamp(-50,50)

        seq_pos_emb = self.seq_sinusoid_emb(seq_pos)
        seq_pos_emb = self.seq_projection(seq_pos_emb)
...
class LayoutLMSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # seq positional encoding
        seq_pos_emb = seq_pos_emb.view(seq_length, seq_length, 1, d_head)
        seq_pos_emb = seq_pos_emb.permute([2, 0, 1, 3]).expand(query_layer.size(0),-1,-1,-1)
        seq_pos_scores = torch.einsum(
            "bnid,bijd->bnij", (query_layer, seq_pos_emb)
        )
        attention_scores = attention_scores + seq_pos_scores
...
class LayoutLMPlusModel(LayoutLMPreTrainedModel):
    def __init__(self, config):
        super(LayoutLMPlusModel, self).__init__(config)
        self.config = config
    ...
    def forward(
        ...
        bbox_pos_emb = self.embeddings.calc_bbox_pos_emb(bbox)
        seq_pos_emb = self.embeddings.calc_1d_pos_emb(position_ids)
